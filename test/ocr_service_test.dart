...
Future<File> createTestImage(String text) async {
  ...
  // 保存到临时文件
  final testFile = File('/tmp/test_ocr_${DateTime.now().millisecondsSinceEpoch}.png');
...
void main() {
  ...
  group('OCR Service Tests', () {
    ...
    test('should handle non-existent file gracefully', () async {
      // 测试处理不存在文件的情况
      final result = await ocrService.recognizeTextFromUri('/non/existent/path.png');
    ...
    test('should recognize Chinese text from real image', () async {
      ...
      final testImagePath = 'test/resource/ocr_test_resorce.jpg';
  ...
  group('File Validation Tests', () {
    ...
    test('should verify file exists before processing', () async {
      ...
      try {
        ...
        // 验证不存在的文件
        final invalidFile = File('/tmp/non_existent_${DateTime.now().millisecondsSinceEpoch}.png');
