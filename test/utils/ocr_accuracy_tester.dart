...
class OcrAccuracyTester {
  ...
  Future<Map<String, dynamic>> runBatchTest({
    ...
    for (int i = 0; i < groundTruth.length; i++) {
      ...
      final emoji = result.accuracy >= 0.8 ? 'PASS' : (result.accuracy >= 0.5 ? 'WARN' : 'FAIL');
      print('[$emoji] Accuracy: ${(result.accuracy * 100).toStringAsFixed(1)}%');
    ...
    final avgAccuracy = results.isNotEmpty ? totalAccuracy / results.length : 0.0;
    final successRate = results.isNotEmpty ? successCount / results.length : 0.0;
    final finalChineseAccuracy = chineseCount > 0 ? chineseAccuracy / chineseCount : 0.0;
    final finalEnglishAccuracy = englishCount > 0 ? englishAccuracy / englishCount : 0.0;
  ...
  void _printReport(Map<String, dynamic> report) {
    ...
    print('  Chinese: ${report['chinese_count']} tests, ${chiAcc.toStringAsFixed(2)}% accuracy');
    print('  English: ${report['english_count']} tests, ${engAcc.toStringAsFixed(2)}% accuracy');
    print('-' * 60);
    
    final sortedResults = List<Map>.from(report['results'])
      ..sort((a, b) => (a['accuracy'] as double).compareTo(b['accuracy'] as double));
    ...
    final failedTests = sortedResults.where((r) => (r['accuracy'] as double) < 0.5).toList();
