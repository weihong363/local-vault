...
class SavePage extends ConsumerStatefulWidget {
  final dynamic initialData; // 可以是 String 或 Map (图片分享)

  const SavePage({super.key, this.initialData});
...
class _SavePageState extends ConsumerState<SavePage> {
    ...
    try {
      ...
      if (widget.initialData != null) {
        ...
        if (widget.initialData is String && widget.initialData!.isNotEmpty) {
          ...
          debugPrint('✅ [SavePage] 从 GoRouter extra 获取分享文本成功：${text.substring(0, math.min(50, text.length))}...');
          setState(() {
      ...
      if (args is String && args.isNotEmpty) {
        debugPrint('✅ [SavePage] 从 ModalRoute 获取分享文本成功：${args.substring(0, math.min(50, args.length))}...');
        setState(() {
    ...
    try {
      ...
      if (result.success && result.text.isNotEmpty) {
        ...
        setState(() {
          ...
          _titleController.text = 'OCR 识别 - ${DateTime.now().toString().substring(0, 16)}';
          _hasInitializedShareText = true;
        });
  ...
  Future<void> _saveSummary() async {
    ...
    try {
      ...
      // 如果有备注，将备注添加到内容中
      final fullContent = remark.isNotEmpty 
          ? '$content\n\n---备注---\n$remark'
          : content;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          Builder(
            builder: (context) {
              final hasShareText = _source == 'share' && _contentController.text.isNotEmpty;
              if (hasShareText) {
                return Container(
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              hintText: '用逗号分隔多个标签',
              hintStyle: TextStyle(color: Colors.grey.shade400),
              prefixIcon: const Icon(Icons.local_offer, color: Colors.purple),
              border: OutlineInputBorder(
                borderRadius: BorderRadius.circular(12),
              ),
              focusedBorder: OutlineInputBorder(
